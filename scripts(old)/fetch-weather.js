...
// data/weather-history.json. Sumber data: Open-Meteo (gratis, tanpa API key).
// Dedup per minggu: kalau minggu berjalan sudah punya entry, entry itu
// di-update (bukan ditambah baru) supaya histori tetap 1 titik/minggu.
...
const MAX_WEEKS = 26; // ~6 bulan riwayat, biar file gak membengkak
...
async function main() {
  ...
  for (const zone of ZONES) {
    try {
      ...
      const hist = store.zones[zone.id] || [];
      const idx = hist.findIndex((h) => h.week === week);
      if (idx >= 0) hist[idx] = entry; else hist.push(entry);
      store.zones[zone.id] = hist.slice(-MAX_WEEKS);
      successCount++;
